...
- Preview decode params: Beam 5 on a GPU (greedy on CPU fallback) + shorter
  window (~8 s instead of 20 s). CUDA preview audio is split into short VAD
  speech spans, avoiding long internal silences that prompt Whisper subtitle
  hallucinations. An aligned rolling-window timeline commits repeated words
  without rewriting them, while the current provisional tail remains visible.
  Matching ignores punctuation-only changes such as `one,` versus `one`.
...
- 2026-07-21: Live preview regression fixed. The `whisper-rs` 0.16
  ...
  visible, but VAD-empty snapshots discard it. CUDA preview uses Beam 5 and
