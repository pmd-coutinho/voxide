...
fn install_parakeet_archive(
    ...
    let install_result = (|| {
        ...
        // Reclaim the dual-precision directory from the reverted fp16 experiment.
        let dual = models_directory.join("parakeet-tdt-0.6b-v2");
        if dual != destination {
            let _ = fs::remove_dir_all(&dual);
        }
        Ok(destination)
    })();
...
async fn download_parakeet_model(
    ...
    let directory = state.models_directory()?;
    let total = parakeet::MODEL_ARCHIVE_BYTES;
    let temporary = directory.join(format!("{}.tar.bz2.download", parakeet::MODEL_ID));
    download_archive_resumable(
        &app,
        parakeet::MODEL_ID,
        parakeet::MODEL_ARCHIVE_URL,
        &temporary,
        total,
        0,
        total,
    ...
    let validation = tauri::async_runtime::spawn_blocking(move || {
        validate_parakeet_archive(&validate_path, Some(total), total)
    })
    .await
...
mod tests {
    ...
    fn parakeet_archive_is_pinned_to_a_digest_and_exact_size() {
        ...
        assert!(parakeet::MODEL_ARCHIVE_URL.starts_with("https://"));
    }
