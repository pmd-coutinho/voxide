...
function renderVoiceEngine(): void {
  ...
  const downloadId = isWhisper ? database.settings.selectedModel : isParakeet ? "parakeet-tdt-0.6b-v2" : isNemotron ? (nemotronRuntimeReady ? "nemotron-3.5-asr-streaming-0.6b" : "nemotron-cuda-runtime") : "";
...
async function handleAction(element: HTMLElement): Promise<void> {
  switch (element.dataset.action) {
    ...
    case "download-parakeet-model": {
      showNotice("Downloading Parakeet TDT 0.6B (fp16 + int8). This is about 1.6 GB and may take a few minutes.");
      modelDownloadProgress = { id: "parakeet-tdt-0.6b-v2", downloadedBytes: 0 };
      render();
      try {
